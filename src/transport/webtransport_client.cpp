...
int apply_pending_writes(WebTransportClient::Impl& impl) {
    ...
    const auto apply_reset = [&](const WebTransportClient::Impl::PendingReset& reset) {
        ...
        if (stream_ctx != nullptr) {
            const int webtransport_result =
                picowt_reset_stream(impl.cnx, stream_ctx, reset.error_code);
            if (webtransport_result == 0 ||
                webtransport_result == PICOQUIC_ERROR_STREAM_ALREADY_CLOSED) {
                return 0;
            }
        }
        const int fallback_result =
            picoquic_reset_stream(impl.cnx, reset.stream_id, reset.error_code);
        return fallback_result == PICOQUIC_ERROR_STREAM_ALREADY_CLOSED
                   ? 0
                   : fallback_result;
    };
    for (const auto& reset : resets) {
        if (apply_reset(reset) != 0) {
            return fail_media_delivery(
                impl, "failed to reset webtransport media stream", -1);
        }
    }
    ...
    for (const auto& reset : activation_resets) {
        if (apply_reset(reset) != 0) {
            return fail_media_delivery(
                impl, "failed to reset activated webtransport media stream", -1);
        }
    }
    ...
    switch (event) {
        ...
        case picohttp_callback_free: {
            if (stream_ctx != nullptr) {
                ...
                // h3zero forwards picoquic_callback_stream_released here, so
                // this is the per-stream all-data-committed notification.
                impl->subgroup_deadlines.mark_all_data_committed(stream_ctx->stream_id);
        ...
        case picohttp_callback_reset: {
            if (stream_ctx != nullptr) {
                std::lock_guard<std::mutex> lock(impl->mutex);
                impl->pending_media.clear_stream(stream_ctx->stream_id);
                impl->subgroup_deadlines.cancel(stream_ctx->stream_id);
                impl->pending_media_activations.erase(stream_ctx->stream_id);
                impl->media_streams_with_pending.erase(stream_ctx->stream_id);
...
std::size_t WebTransportClient::peer_stopped_media_stream_count_for_testing()
    const {
    if (impl_ == nullptr) {
        return 0;
    }
...
bool WebTransportClient::connection_close_sent_for_testing() const {
    if (impl_ == nullptr) {
        return false;
    }
    std::lock_guard<std::mutex> lock(impl_->mutex);
    return impl_->close_sent;
}
