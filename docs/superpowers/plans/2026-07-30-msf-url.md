...
Declare `bool transport_set = false;` alongside the other `_set` locals, and set it in the existing `--transport` branch:
...
Also guard the reverse order, where `--endpoint` follows `--url`. In the `--endpoint` branch, before assigning:
...
Add this in `main()` immediately after options are parsed and before the ingest
branches, so it runs identically on every path:
...
```cpp
    {
        const auto options = parse({"prog", "--endpoint", "h:443", "--print-msf-urls"});
        ok &= expect(options.print_msf_urls, "expected --print-msf-urls to set the flag");
        const auto defaulted = parse({"prog", "--endpoint", "h:443"});
...
Then the deviation guard. Build a catalog carrying a content protection entry whose `la_url` is percent-encoded, following whatever construction `tests/msf_catalog_test.cpp` already uses for `content_protections` (Phase 3 added those cases to this file; reuse that block's shape rather than inventing a new one):
...
```cpp
    ...
    // under RFC 3986. Pinned so it cannot regress silently.
    {
        ...
        attach_content_protection(catalog, catalog.tracks.front().name, protection);
...
`attach_content_protection` and the exact required fields of `MsfContentProtection` come from Phase 3; check `include/openmoq/publisher/msf_catalog.h` for its signature and populate whatever fields its validation requires. Report in your report file what you had to add.
