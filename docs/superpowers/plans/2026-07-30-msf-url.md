...
**Three existing statements assert Phase 4 is unimplemented. All three must change, or the file contradicts itself** — the exact defect the Phase 3 review caught between `status.md:58` and `:64`:

1. `docs/status.md:58` — "and Phase 4, MSF URL parsing, remains."
2. `docs/status.md:91` — "MSF URL and fragment parsing (Phase 4)" inside a list of things not implemented.
3. `docs/protocol-mapping.md:119` — "MSF section 11.1 URL parsing" inside a comparable not-implemented list.

Line numbers will have shifted by the time you edit; find each by its text. Afterwards, grep both files for `Phase 4` and for `11.1` and read every hit to confirm none still claims the work is pending.

MSF section 12 encryption fields, MSF section 12 compression signalling, `clone` delta operations, and the CMSF 4.1.1.4.4 Authorization URL remain genuinely unimplemented and must stay in those lists. Remove only the Phase 4 URL items.

- [ ] **Step 2: Record the deviations in `docs/protocol-mapping.md`**

The file is organised as `## Draft-14 primary assumptions`, `## Draft-16 secondary assumptions`, `## CMAF packaging assumptions`, `## MSF v1 catalog`, `## CMSF content protection`, `## Implementation consequence`. Add a new `## MSF URLs and fragments` section **after `## CMSF content protection` and before `## Implementation consequence`**, matching that progression from transport to packaging to catalog to feature areas. Note that the stale "MSF section 11.1 URL parsing" line lives inside the earlier `## MSF v1 catalog` section, so it is edited separately from where the new section goes.
