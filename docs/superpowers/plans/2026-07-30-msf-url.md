...
void validate_no_stray_percent(const std::string& value, std::string_view field,
    ...
    for (std::size_t index = 0; index < value.size(); ++index) {
        ...
        if (closing == std::string::npos) {
            throw std::runtime_error("track " + track_name + " field " + std::string(field) +
                                     " has an unterminated '%' variable reference");
        }
...
void validate_optional_no_stray_percent(const std::optional<std::string>& value,
                                        std::string_view field, const std::string& track_name) {
    if (value.has_value()) {
        validate_no_stray_percent(*value, field, track_name);
    }
}
```
...
```cpp
    validate_no_stray_percent(track.name, "name", track.name);
    validate_no_stray_percent(track.packaging, "packaging", track.name);
    validate_optional_no_stray_percent(track.name_space, "namespace", track.name);
    validate_optional_no_stray_percent(track.role, "role", track.name);
    validate_optional_no_stray_percent(track.label, "label", track.name);
    validate_optional_no_stray_percent(track.init_ref, "initRef", track.name);
    validate_optional_no_stray_percent(track.codec, "codec", track.name);
    validate_optional_no_stray_percent(track.mime_type, "mimeType", track.name);
    validate_optional_no_stray_percent(track.channel_config, "channelConfig", track.name);
    validate_optional_no_stray_percent(track.lang, "lang", track.name);
    validate_optional_no_stray_percent(track.event_type, "eventType", track.name);
    for (const auto& dependency : track.depends) {
        validate_no_stray_percent(dependency, "depends", track.name);
    }
```
